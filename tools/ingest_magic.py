...
def main():
    ...
    # Provenance header, source-aware. The firmware set's raw descriptions are
    # later re-authored clean-room by reauthor_firmware_desc.py, so the shipped
    # firmware.toml reproduces no third-party prose. The general set instead
    # retains file(1)'s Magdir description labels verbatim (BSD-style), so that
    # source + license is noted on it.
    from_fkie = any("firmware-magic-database" in d for d in magdirs)
    if from_fkie:
        src_line = "# GENERATED by tools/ingest_magic.py from the fkie firmware-magic-database."
    else:
